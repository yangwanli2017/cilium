...
public:
  TestConfig(const Json::Object& config, Stats::Scope& scope) : Config(config, scope) {
    original_dst_address_ = Network::Utility::parseInternetAddressAndPort(config.getString("original_dst_address"));
...
public:
  TestInstance(TestConfigSharedPtr json) : Instance(json), test_config_(json.get()) {}
...
public:
  ...
  ListenerFilterFactoryCb createFilterFactory(const Json::Object& json, ListenerFactoryContext& context) override {
    Filter::BpfMetadata::TestConfigSharedPtr config(new Filter::BpfMetadata::TestConfig(json, context.scope()));

    return [config](Network::ListenerFilterManager& filter_manager) mutable -> void {
      filter_manager.addAcceptFilter(std::make_shared<Filter::BpfMetadata::TestInstance>(config));
    };
  }
 ...
 */
static Registry::RegisterFactory<TestBpfMetadataConfigFactory, NamedListenerFilterConfigFactory> registered_;

} // namespace Configuration
} // namespace Server

class CiliumIntegrationTest : public BaseIntegrationTest,
...
public:
  ...
  void Denied(Http::TestHeaderMapImpl headers) {
    ...
    codec_client = makeHttpConnection(lookupPort("cilium"), Http::CodecClient::Type::HTTP1);
  ...
  void Accepted(Http::TestHeaderMapImpl headers) {
    ...
    request_stream->encodeHeaders(Http::TestHeaderMapImpl{{":status", "200"}}, true);
    response->waitForEndStream();
...
INSTANTIATE_TEST_CASE_P(IpVersions, CiliumIntegrationTest,
