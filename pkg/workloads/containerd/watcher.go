...
func EnableEventListener() error {
	...
	pkgLog.Debug("Started to listen for containerd events")
	return nil
}
...
func listenForDockerEvents(ws *watcherState, reader io.ReadCloser) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(scanner.Bytes(), &e); err != nil {
			pkgLog.WithError(err).Error("Error while unmarshalling event")
		}
	...
	if err := scanner.Err(); err != nil {
		pkgLog.WithError(err).Error("Error while reading events")
	}
}
...
func getCiliumEndpointID(cont *dTypes.ContainerJSON) uint16 {
	scopedLog := pkgLog.WithField(logfields.ContainerID, shortContainerID(cont.ID))
...
func getCiliumIPv6(networks map[string]*dNetwork.EndpointSettings) *addressing.CiliumIPv6 {
	for _, contNetwork := range networks {
		...
		scopedLog := pkgLog.WithField(logfields.EndpointID, contNetwork.EndpointID)
...
func getFilteredLabels(allLabels map[string]string) (identityLabels, informationLabels labels.Labels) {
	...
	if podName := k8sDockerLbls.GetPodName(allLabels); podName != "" {
		...
		if err != nil {
			pkgLog.WithError(err).Warn("Error while getting Kubernetes labels")
...
func IgnoreRunningContainers() {
	...
	for _, cont := range conts {
		scopedLog := pkgLog.WithField(logfields.ContainerID, cont.ID)
