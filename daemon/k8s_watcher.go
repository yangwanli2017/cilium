	...
	cnpCM.UpdateController(ctrlName,
		controller.ControllerParams{
			DoFunc: func() error {
				...
				if err2 != nil {
					scopedLog.WithError(err2).Warn("Unable to find v1.CiliumNetworkPolicy in local cache")
					// Stopping this controller, we have logged the error though
					return nil
				}
				if !exists {
					// Stopping this controller since the policy doesn't exist.
					return nil
				}
...
func (d *Daemon) deleteCiliumNetworkPolicyV1(cnp *cilium_v1.CiliumNetworkPolicy) {
	...
	ctrlName := cnp.GetControllerName()
	err := cnpCM.RemoveController(ctrlName)
	if err != nil {
