...
func ParseNode(k8sNode *v1.Node) *node.Node {
	...
	for _, addr := range k8sNode.Status.Addresses {
		...
		if ip == nil {
			scopedLog.WithFields(log.Fields{
				...
				"type":           addr.Type,
			}).Debug("k8s: Ignoring invalid node IP")
			continue
		}
		na := node.Address{
	...
	if len(k8sNode.Spec.PodCIDR) != 0 {
		if _, cidr, err := net.ParseCIDR(k8sNode.Spec.PodCIDR); err != nil {
			scopedLog.WithError(err).WithField(logfields.V4Prefix, k8sNode.Spec.PodCIDR).Warn("k8s: Invalid PodCIDR value for node")
		} else {
			if cidr.IP.To4() != nil {
	...
	if node.IPv4AllocCIDR == nil {
		if ipv4CIDR, ok := k8sNode.Annotations[Annotationv4CIDRName]; !ok {
			scopedLog.Debug("k8s: Empty IPv4 CIDR annotation in node")
		} else {
			_, cidr, err := net.ParseCIDR(ipv4CIDR)
			if err != nil {
				scopedLog.WithError(err).WithField(logfields.V4Prefix, ipv4CIDR).Error("k8s: BUG, invalid IPv4 annotation CIDR in node")
			} else {
				node.IPv4AllocCIDR = cidr
			}
		}
	...
	if node.IPv6AllocCIDR == nil {
		if ipv6CIDR, ok := k8sNode.Annotations[Annotationv6CIDRName]; !ok {
			scopedLog.Debug("k8s: Empty IPv6 CIDR annotation in node")
		} else {
			_, cidr, err := net.ParseCIDR(ipv6CIDR)
			if err != nil {
				scopedLog.WithError(err).WithField(logfields.V6Prefix, ipv6CIDR).Error("k8s: BUG, invalid IPv6 annotation CIDR in node")
			} else {
				node.IPv6AllocCIDR = cidr
			}
		}
