...
public:
  ...
  ListenerFilterFactoryCb createFilterFactory(const Json::Object& json,
...
Config::Config(const Json::Object& config, Stats::Scope& scope)
  : bpf_root_(config.getString("bpf_root", "/sys/fs/bpf")),
    ...
    identity_(config.getInteger("identity", 0)),
...
Instance::onAccept(Network::ListenerFilterCallbacks& cb) {
  ...
  if (!getBpfMetadata(socket)) {
    ...
    ENVOY_LOG(info, "bpf_metadata ({}): GOT bpf metadata for new connection (mark: {:x})",
