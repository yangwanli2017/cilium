...
ProxyMap::Proxy4Map::Proxy4Map() : Bpf(BPF_MAP_TYPE_HASH, sizeof(struct proxy4_tbl_key), sizeof(struct proxy4_tbl_value)) {}

ProxyMap::Proxy6Map::Proxy6Map() : Bpf(BPF_MAP_TYPE_HASH, sizeof(struct proxy6_tbl_key), sizeof(struct proxy6_tbl_value)) {}

ProxyMap::ProxyMap(const std::string& bpf_root, Filter::BpfMetadata::Config& parent) : parent_(parent) {
  ...
  ENVOY_LOG(info, "bpf_metadata: Created proxymap for {}", parent_.is_ingress_ ? "ingress" : "egress");
}
...
bool ProxyMap::getBpfMetadata(Network::AcceptSocket& socket) {
  ...
  if (local_address->type() == Network::Address::Type::Ip &&
    ...
    if (ip->version() == Network::Address::IpVersion::v4 && rip->version() == Network::Address::IpVersion::v4) {
      ...
      ENVOY_LOG(info, "getBpfMetadata: Looking up key: {:x}, {:x}, {:x}, {:x}, {:x}", ntohl(key.saddr), ntohs(key.dport), ntohs(key.sport), key.nexthdr, key.pad);
	...
	Network::Address::InstanceConstSharedPtr orig_local_address = std::make_shared<Network::Address::Ipv4Instance>(&ip4);
	socket.resetLocalAddress(orig_local_address);
	socket.setSocketMark(parent_.getMark(value.identity));
	return true;
      }
      ENVOY_LOG(info, "getBpfMetadata: IPv4 bpf map lookup failed: {}", strerror(errno));
    } else if (ip->version() == Network::Address::IpVersion::v6 && rip->version() == Network::Address::IpVersion::v6) {
	...
	Network::Address::InstanceConstSharedPtr orig_local_address = std::make_shared<Network::Address::Ipv6Instance>(ip6);
	socket.resetLocalAddress(orig_local_address);
	socket.setSocketMark(parent_.getMark(value.identity));
	return true;
      }
      ENVOY_LOG(info, "getBpfMetadata: IPv6 bpf map lookup failed: {}", strerror(errno));
    } else {
      ENVOY_LOG(info, "getBpfMetadata: Address type mismatch: Source: {}, Dest: {}", rip->addressAsString(), ip->addressAsString());
    }
  }
