...
public:
   ...
   * @param map_type the type of a bpf map to be opened or created, e.g., BPF_MAP_TYPE_HASH
  ...
  /**
   * Open an existing bpf map. The bpf map must have the map type and key and value sizes that
   * match with the ones given to the constructor.
   ...
   * @param max_entries the maximum capacity of the bpf map to be created. For many map types
   *        memory for this number of entries is allocated when the map is created.
   ...
   * @param value pointer to the value to be stored in the new entry to be inserted.
   * @returns boolean for success of the operation.
   */  
  bool insert(const void *key, const void *value);

  /**
   * Lookup an entry from the bpf map identified with the key, storing the found value, if any.
